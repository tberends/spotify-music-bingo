...
from .constants import (
    ...
    DEFAULT_DURATION_BETWEEN_CLIPS,
)
...
@click.option(
    "--players",
    default="1",
    help="Number of bingo cards to make or a CSV list of player names.",
)
@click.option(
    "--track-count",
    default=DEFAULT_TRACK_COUNT,
    help="The number of tracks to use from the playlist.",
)
@click.option(
    "--debug",
    is_flag=True,
    default=False,
    help="Whether or not to output additional logging.",
)
def generate_cards(playlist, game_id, players, track_count, debug):
    try:
        ...
        click.secho("ERROR: The specified playlist does not exist.", fg="red")
        return 1
    ...
    click.secho(
        generate_play_game_command(playlist, game.game_id, track_count), fg="green"
    )
    click.secho("\n")
...
@click.option(
    ...
    help="How long should the song clip play for?",
)
@click.option(
    "--duration-between-clips",
    default=DEFAULT_DURATION_BETWEEN_CLIPS,
    help="The number of seconds of slience between clips",
)
@click.option(
    "--starting-track",
    default=0,
    help="The track number to start from. Useful with --game-id for resuming a game.",
)
@click.option(
    "--track-count",
    default=DEFAULT_TRACK_COUNT,
...
def play_game(
    ...
    if len(devices) > 1:
        ...
        value = click.prompt(
            "Enter a device number ({}-{}) to play on".format(0, len(devices) - 1),
            type=int,
        )
    ...
    click.secho(
        "Using playlist {} with {} tracks.".format(
            bingo_playlist.name(), len(bingo_playlist.tracks())
        )
    )
    ...
    try:
        ...
        click.secho(
            generate_play_game_command(
                playlist, game_id, track_count, game.current_track
            ),
            fg="green",
        )
        click.secho("\n")
    ...
    if game.end_status == "PLAYLIST_COMPLETE":
        click.secho("Game ended. All tracks have been played.", fg="yellow")
